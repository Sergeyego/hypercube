    ...
    ui(new Ui::CubeWidget)
{
    ...
    connect(olapmodel,SIGNAL(sigUpd()),ui->tableView,SLOT(resizeColumnsToContents()));
...
{
    ui->tableView->saveXls(this->windowTitle()+tr(" с ")+ui->dateEditBeg->date().toString("dd.MM.yy")+tr(" по ")+ui->dateEditEnd->date().toString("dd.MM.yy"));
}
